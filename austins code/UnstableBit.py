...
file_path = r"C:\Users\afeig\Documents\school\Current_Semester\ECE-401\Garbage_Values_150nm-1\30_captures_100ms_delay_capacitor.txt"
BITS_PER_WORD = 16
...
# Create a custom colormap (White -> Red -> Blue -> White)
colors = ["white", "red", "darkblue", "white"]
cmap = ListedColormap(colors)

# Define color boundaries for the instability (0-40% white, 40-50% red, 50-60% dark blue, 60-100% white)
bounds = [0, 0.4, 0.5, 0.6, 1]
norm = plt.Normalize(vmin=0, vmax=1)

# Apply the custom colormap and boundaries
plt.imshow(instability_image_upscaled, cmap=cmap, norm=norm)

# Add a color bar to indicate the intensity of the instability
plt.colorbar(label='Bit instability (normalized)', ticks=[0, 0.4, 0.5, 0.6, 1])

# Save and show the image
plt.savefig("bit_instability_colormap_visualization.png", dpi=20)
plt.show()
